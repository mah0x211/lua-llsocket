...
-- dgram socket
fd = ifNil( isolate( inet.bind, HOST, PORT, DGRAM, nil, REUSE ) );
-- dgram socket cannot listen
ifNil( llsocket.listen( fd ) );
ifNotNil( llsocket.close( fd ) );
-- w/o port
fd = ifNil( isolate( inet.bind, HOST, nil, DGRAM, nil, REUSE ) );
ifNotNil( llsocket.close( fd ) );
-- w/o host
fd = ifNil( isolate( inet.bind, nil, PORT, DGRAM, nil, REUSE ) );
ifNotNil( llsocket.close( fd ) );
-- w nonblock
fd = ifNil( isolate( inet.bind, HOST, nil, DGRAM, NONBLOCK, REUSE ) );
ifNotTrue( llsocket.opt.nonblock( fd ) );
ifNotNil( llsocket.close( fd ) );
...
ifNotNil( llsocket.close( fd ) );
-- w/o port
fd = ifNil( isolate( inet.bind, HOST, nil, RAW, nil, REUSE ) );
ifNotNil( llsocket.close( fd ) );
-- w/o host
fd = ifNil( isolate( inet.bind, nil, PORT, RAW, nil, REUSE ) );
ifNotNil( llsocket.close( fd ) );
-- w nonblock
